...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	const int colourThreshold = 1;
	const int searchRadius = 1200;
	const int sizeThreshold = 1000;
	...
	for( int frame = 0; frame < numOfImages; frame++)
	{
		...
		if( frame > 0 )//every frame but initial
		{
			...
			//fillSearchArea( int Xori, int Yori, int searchRadius, vector< Point >* searchArea, const int yMax, const int xMax );
			int Xori, Yori, dX, dY;

			for( unsigned int targetNumber = 0; targetNumber < target[frame].size(); targetNumber++ )
			{
				Xori = target[frame][targetNumber].xMidF();
				Yori = target[frame][targetNumber].yMidF();

				for( unsigned int i = 0; i < target[frame-1].size(); i++ )
				{
					Point mid;
					mid.x = target[frame-1][i].xMidF();
					mid.y = target[frame-1][i].yMidF();
					dX = mid.x - Xori;
					dY = mid.y - Yori;
					
					TargetID curTarget;

					//checks that the target from frame-1 is within searchRadius
					int size0 = target[frame-1][i].targetPixelCount();
					int size1 = target[frame][targetNumber].targetPixelCount();
					bool a = ( size0 <= size1 + sizeThreshold ) && ( size0 >= size1 - sizeThreshold );
					if( (dX*dX + dY*dY <= searchRadius*searchRadius) && a )
					{
						curTarget.targetNum = i;
						curTarget.mid = mid; 
						target[frame][targetNumber].addRelatedTarget( curTarget );
						printf("%i,%i,%i,%i\n", targetNumber, i, mid.x, mid.y);
					}
				}
			}
		}
...
void colourImage( ImageAttr* attr_img, const vector< Target >& target )
{
	...
	for( vector< Target >::size_type i = 0; i < target.size(); i++ )//for every target
	{
		...
		//draws line between tarrget and related target
		int numRT = target[i].numRelatesTargets();//related targets
		
		for( int n = 0; n < numRT; n++ )
		{
			int x0 = target[i].xMidF();
			int y0 = target[i].yMidF(); 
			
			int x1 = target[i].relatedTargetAt( n ).x;
			int y1 = target[i].relatedTargetAt( n ).y;
